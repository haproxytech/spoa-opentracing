...
static int check_capabilities_cb(struct spoe_frame *frame, void *arg1, void *arg2)
{
	...
	while (len > 0) {
		...
		if (STR_CMP_GE(STR_CAP_FRAGMENTATION, str, len)) {
		...
		else if (STR_CMP_GE(STR_CAP_PIPELINING, str, len)) {
		...
		else if (STR_CMP_GE(STR_CAP_ASYNC, str, len)) {
...
static void use_spoe_engine(struct client *client)
{
	...
	if (_NULL(e = MEM_CALLOC(1, sizeof(*e)))) {
		client->async = false;
	...
	e->id = MEM_STRDUP(client->engine_id);
	LIST_INIT(&(e->clients));
