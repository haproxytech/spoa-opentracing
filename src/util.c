...
void *mem_dup(const void *s, size_t size)
{
	...
	if (_NULL(retptr = MEM_MALLOC(size + 1)))
		return retptr;
...
void buffer_init(struct buffer *data)
{
	...
	DBG_BUFFER("  initializing buffer ", data);
...
void buffer_free(struct buffer *data)
{
	...
	DBG_BUFFER("  freeing buffer ", data);
...
struct buffer *buffer_alloc(size_t size, const void *src, ...)
{
	...
	if (_NULL(retptr = MEM_CALLOC(1, sizeof(*retptr)))) {
		/* Do nothing. */
	}
	else if ((size > 0) && _NULL(retptr->ptr = MEM_CALLOC(1, size))) {
		MEM_FREE_CLEAR(retptr);
	}
	else {
...
ssize_t buffer_grow(struct buffer *data, const void *src, size_t n)
{
	...
	if (n == 0) {
		/* If the data length is 0, nothing is added. */
		retval = data->len;
	}
	...
	else if (_NULL(ptr = MEM_REALLOC(data->ptr, data->size + size))) {
...
int thread_mutex_init(pthread_mutex_t *mutex)
{
	...
	DBG_FUNC(NULL, "%p", mutex);

	if (_NULL (mutex))
		return retval;

	if (_OK (retval = pthread_mutexattr_init (&attr)))
		if (_OK (retval = pthread_mutexattr_settype (&attr, PTHREAD_MUTEX_RECURSIVE)))
			if (_OK (retval = pthread_mutexattr_setrobust_np (&attr, PTHREAD_MUTEX_ROBUST_NP)))
 ...
 *   -
 */
char *file_read(const char *filename, bool_t flag_trim_comments)
{
	struct stat  statbuf;
	char        *retptr = NULL;
	int          fd, rc;

	DBG_FUNC(NULL, "\"%s\", %hhu", filename, flag_trim_comments);

	if (_NULL(filename))
		return retptr;

	if ((fd = open(filename, O_RDONLY)) == -1) {
		(void)fprintf(stderr, "ERROR: unable to open file '%s': %s\n", filename, strerror(errno));
	}
	else if ((rc = fstat(fd, &statbuf)) == -1) {
		(void)fprintf(stderr, "ERROR: cannot stat file '%s': %s\n", filename, strerror(errno));
	}
	else if (_NULL(retptr = MEM_MALLOC(statbuf.st_size + 1))) {
		(void)fprintf(stderr, "ERROR: cannot alloc memory: %s\n", strerror(errno));
	}
	else {
		char    *buf = retptr;
		...
		while (size > 0) {
			...
			if ((n = read(fd, buf, size)) > 0) {
				...
				if (TEST_OR2(errno, EAGAIN, EWOULDBLOCK)) {
					break;
				}
				else if (errno != EINTR)
				{
					(void)fprintf(stderr, "ERROR: read error '%s': %s\n", filename, strerror(errno));
					MEM_FREE_CLEAR(retptr);
		...
		if (flag_trim_comments) {
			...
			W_DBG(UTIL, NULL, "  read %ld (%ld) byte(s): ok", statbuf.st_size - size, n);
		} else {
			if (size != 0)
				MEM_FREE_CLEAR(retptr);

			W_DBG(UTIL, NULL, "  read %ld of %ld byte(s): %s", statbuf.st_size - size, statbuf.st_size, (size == 0) ? "ok" : "error");
		}
	}
